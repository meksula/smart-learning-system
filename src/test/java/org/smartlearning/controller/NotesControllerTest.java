...
public class NotesControllerTest {
    ...
    public void shouldRedirectToView() throws Exception {
        MockMvc mockMvc = standaloneSetup(notesController).build();

        mockMvc.perform(get("/notes").principal(createPrincipal()))
                .andExpect(model().attributeExists("username"))
                .andExpect(view().name("redirect:/notes/{username}"));
    }
    ...
    public void shouldPresentCorrectlyView() throws Exception {
        ...
        mockMvc.perform(get("/notes/{username}", username).principal(createPrincipal()))
